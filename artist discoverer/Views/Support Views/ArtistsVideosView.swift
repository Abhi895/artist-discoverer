...
struct ArtistsVideosView: View {
    ...
    // Helper to get URLs (Same as your original logic)
    private var videoURLs: [URL] {
        var urls: [URL] = []
        if let videosFromRoot = Bundle.main.urls(forResourcesWithExtension: "mp4", subdirectory: nil) {
            for artist in videoManager.following {
                for url in videosFromRoot {
                    if url.lastPathComponent.contains(artist.artistName) {
                        urls.append(url)
                    }
                }
            }
    ...
    var body: some View {
                ...
                VStack {
                    ...
                    NavigationLink(value: ArtistPlaylist(initialIndex: index, videos: videoURLs)) {
                        ...
                        ZStack(alignment: .bottomLeading) {
                            ...
                            VideoRowView(url: url, index: index, preview: true, followingOnly: false)
        ...
        .navigationDestination(for: ArtistPlaylist.self) { playlist in
            ...
            SongsVideosView(
                currentVideoIndex: playlist.initialIndex,
                customVideoURLs: playlist.videos,
            )
        }
